...
const InputForm = () => {
  ...
  const generateQRCodeData = () => {
    ...
    setQrCodeData(JSON.stringify(newData));
  };
    ...
    <div className="col-span-3 p-6 grid gap-4" style={{ gridTemplateColumns: "1fr 1fr" }}>
      <div>
        ...
        <div className="text-center px-10 py-10">
          <Generat setQrCodeData={generateQRCodeData} />
        </div>
      </div>
          ...
          {qrCodeData ? (
            <div>
              <QRCode
                ...
                level="H"
              />
              <p className="mt-4">This is the QR code of: {text}</p>
              {logo && <img src={URL.createObjectURL(logo)} alt="Uploaded Logo" className="mt-4 max-h-40" />}
              <button
